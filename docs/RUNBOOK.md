...
- (2026-07-18: 전 11개 트레이드 de-tell 완료 — 한시 예외 조항 해제됨. 게이트 2는 이제 무조건 ALL PASS 필수.)
...
- ⚠️ **체인 개명 함정(2026-07-17 실전 사례)**: A→B→C 슬러그 체인 개명 시 중간 노드(B)는 소스이자 목적지다. "모든 옛 슬러그를 스텁으로" 일괄 처리하면 방금 쓴 새 콘텐츠를 덮어쓴다 — 스텁 대상 = (소스 집합 − 목적지 집합)만. 사고 시 복구는 `git show HEAD:<file>`.
